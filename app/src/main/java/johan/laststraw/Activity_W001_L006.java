 ...
 */
public class Activity_W001_L006 extends Activity implements View.OnClickListener, Animation.AnimationListener {
    ...
    String enemyCard1Name = "Speed Up", enemyCard2Name = "Slow Down", enemyCard3Name = "Slow Down", enemyCard4Name = "Reinforce III",
        ...
        switch (v.getId()) {
            case R.id.ibPlayerCard1:
                ...
                if (playerCard1Type != 0 && playerMoves >= getCardCost() + playerCorruptedPenalty && objectsRemaining != 0) {
                    ...
                    playCardConfirm(message, Activity_W001_L006.this);
                }
                break;
    ...
    private void gameOver() {
        ...
        myHandler.postDelayed(new Runnable() {
            public void run() {
                final Dialog dialog = new Dialog(Activity_W001_L006.this);
    ...
    private void checkLevelInfo(){
        if (lvlcleared == 0){
            ...
            if (finalPlayerScore > lvlhighscore){
                cursor = db.getLvlInfo(6);
                if (cursor != null && cursor.moveToFirst()) {
                    if (playerWon){
                        db.updateLvlInfo(6, 1, finalPlayerScore);
                    } else {
                        db.updateLvlInfo(6, lvlcleared, finalPlayerScore);
                    }
                }
            } else {
                cursor = db.getLvlInfo(6);
                if (cursor != null && cursor.moveToFirst()) {
                    if (playerWon){
                        db.updateLvlInfo(6, 1, lvlhighscore);
                    } else {
                        db.updateLvlInfo(6, lvlcleared, lvlhighscore);
                    }
                }
            }
    ...
    *  -----------------------------------------*/
    ...
    private void coinFlipStart(){
        ...
        myHandler.postDelayed(new Runnable() {
            public void run() {
                chooseCoinSide(Activity_W001_L006.this);
            }
        }, 1500);
    }
